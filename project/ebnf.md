...
DISPLAY ::= 'show ' + KEY + ((', ' + KEY )? + ' and ' + KEY)? // I would like the Oxford comma :P
...
S_OP ::= ('includes || 'does not include') || ('begins' || 'does not begin' || 'ends' || 'does not end') + ' with'
AGGREGATOR ::= 'MAX' || 'MIN' || 'AVG' || 'SUM'
TYPE ::= 'courses' || 'rooms'

KEY ::= COURSE_KEY || ROOM_KEY
